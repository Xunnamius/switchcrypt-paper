...
\hank{I noticed two issues here with the introduction of these case
  studies in the intro and their presentation here.  First, they are
  presented in two different orders.  Second, they use two different
  names without tying them together.  The intro order is forward,
  selective, mirrored.  Here is is battery, upgrade, scalable.  My
  suggestion is to use the same orderig in both cases and then remind
  readers in this section why these case studies exist. So, it should
  be battery needs forward switching, different file security needs
  selective, and upgrade needs mirrored.  In other words, use the same
  ordering everywhere (later in the paper as well) and remind readers
  that each case study connects to a technical contribution of the
  paper.  Added after reading further, so it looks like the ordering
  is just inconsistent everywhere.  In section2 and section 3, the
  orderings match, but the intro and the usecases subsection swap
  mirrored and selective.  I think I like the ordering here and in
  section 3, so let's keep that and change the intro and usecases
  subsection to match. }
