...
\TODO{You need some sort of overview paragraph that tells people what is to come in this section and why they should care about it.
For example, you want to say something about these case studies cover a wide range of situations including... They also demonstrate uses of both temporal and spatial switching... Basically, make the argument that these case studies provide good coverage of all the things that you discussed earlier in the paper.}
...
frequencies and 2) using \texttt{taskset} to transition the SwitchCrypt processes
...
\begin{figure}[ht] \textbf{Batter Saver Use Case: Energy-Security Tradeoff vs
   ...
   \centering
   {\input{charts/usecase-battery.tex}} \caption{Median sequential write total
...
finish before the devices dies. \TODO{Add that when we get back to a charger, SwitchCrypt can easily recrypt the parts in ChaCha8 back to Freestyle Balanced?}
...
encryption'' has been explored in related work~\cite{ScalableEncryption}. \TODO{If you mention this prior work here, you need to say more about what the difference is. Like prior work used a similar idea for streaming data, but not for data at rest?  I don't know how they handled their metadata now that I think about it.}
...
sensitive information exists within several much larger files. \TODO{The "file or files" language detracts from this section a bit.  It would be stronger if you just stuck with one.  Can you actually support different regions within a file?  If so, that is much stronger in my mind.  Even saying you simulate it would be better to me.}
...
SwitchCrypt instance using Selective switching with a 3:1 ratio of ChaCha8 nugget
...
Freestyle Balanced. Between them, we see SwitchCrypt Selective switching 3:1 ratio
I/O results.
...
SwitchCrypt is made aware when the backing store is in such a state, we can offset
...
We begin by writing 10 40MB files to SwitchCrypt per each cipher as a baseline. We
then introduce a delay into SwitchCrypt I/O of $20ms$ and repeat the experiment
three times.
...
irrecoverable. We show that, given a device that support SSD ISE, SwitchCrypt, and
...
\TODO{Consider adding some kind of lessons learned at the end if you can think of some "pithy" lessons here.}
