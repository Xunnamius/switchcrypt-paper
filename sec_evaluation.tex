...
\Cref{fig:eval-forward} shows the tradeoff between trade score
...
baseline crypts (\ie{7:3, 5:5, and 3:7 described above}).
%Ratio points above baseline measure overhead.
...
\TODO{Y\%/Z\%} (read/write) for 40MB, 5MB and 512KB workloads compared to
baseline I/O, demonstrating \sys's amortization of switching costs. Average
overhead is \TODO{Y\%/Z\%} for 4KB workloads, where \sys is unable to amortize
cost. Similarly, we calculate that Selective switching has average overhead at
\TODO{0\%/Z\%} for 40MB, 5MB and 512KB workloads compared to baseline I/O.
Average overhead is \TODO{Y\%/Z\%} for 4KB workloads. Finally, we calculate that
Mirrored switching has average overhead at \TODO{Y\%/Z\%} for 40MB, 5MB and
512KB workloads compared to baseline I/O, with high write latency due to
mirroring. Average overhead is \TODO{Y\%/Z\%} for 4KB workloads.
...
cost of at most \TODO{Y\%/Z\%} increased read/write overhead (excluding
