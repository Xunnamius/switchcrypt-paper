...
\begin{enumerate}
  \item What shape does the cipher configuration space take under our workloads?
  (\cref{subsec:eval-baseline})
...
is initialized using the primary crypt. Once we trigger a switch, we switch to
the secondary crypt. The switch is triggered according to a certain {\em ratio}
of I/O operations. For example: given 10 40MB ``write-read'' (write and then
read back) operations, we may write-read 40MB 3 times using the primary crypt,
initiate a switch, and then write-read 40MB 7 times. This would be a 3:7 ratio.
There are three ratios we use to evaluate \sys in this way: 7:3, 5:5, and 3:7.
...
\Cref{fig:eval-baseline} shows the tradeoff between trade score (see
\cref{subsec:des-discussion}) and median normalized latency of
different crypts for our sequential and random I/O
