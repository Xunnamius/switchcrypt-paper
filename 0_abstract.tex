...
requirements.  We then present SwitchBox, a software mechanism that enables
...
energy use for desired security properties. We implement SwitchBox on an ARM
...
different switching strategies would be optimal. We then study SwitchBox's
dynamic flexibility through \TODO{XXX} case studies where latency, energy, and
desired security properties change dynamically. In all cases, we find that
SwitchBox achieves at least \TODO{XXX} improvement compared to static approaches
that must pick only a single operating point. \TODO{We need a much stronger
statement about the results, but let's wait until all the case studies are in
before we construct that sentence.}
