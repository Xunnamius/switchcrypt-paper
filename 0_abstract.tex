...
Recent work with stream cipher shows significantly improved performance over
block cipher while offering stronger security properties for Full Drive
Encryption. However, optimizing for performance often conflicts with other key
concerns like energy usage and encryption strength. In this paper we present
SwitchCrypt, a software mechanism that enables navigating the tradeoff space made
by balancing competing security and latency requirements via \emph{cipher
switching} in space or time. We implement SwitchCrypt on an ARM big.LITTLE mobile
