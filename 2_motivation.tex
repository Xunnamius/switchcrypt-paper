...
storage units efficiently. This requires we decouple cipher implementations from
the encryption process. Second, we need to determine when to re-encrypt those
units and where to store the output. We address this with switching strategies.
Third, we must reason about which cipher configurations are most desirable in
which contexts and why. This requires we quantify the desirable properties of
