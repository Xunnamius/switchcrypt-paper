...
For performance reasons, \sys recommends a log-based file
system such as F2FS, \xxx, or \xxx, which are commonly used for flash
devices.  The reason for this is that supporting streaming chipers is more
efficient in storage systems with no in-place updates, as mentioned in the
literature \cite{x, x}.  
\sys is a software solution, however the same logic can be adopted
to storage devices in the future, especially flash devices. 
For example, the no in-place update of the FTL nature will help stream chipers
be performance while at the same time users can use other in-place update file systems
such as ext4, btrfs. 
