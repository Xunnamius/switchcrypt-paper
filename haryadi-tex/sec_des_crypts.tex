...
- ``interface'' -- interface between what and what?  between the OS and \sys?
  between \sys and the cipher configuration? who calls the xorInterface, read/writeInterface? \\
- ``\sys internals'' \\
}
...
% single enc/dec model, the OS has one way to talk to \sys 
...
\mysub{\texttt{xor\_interface}} executes independently of SwitchCrypt
