...
regard: 1:3, 1:1, and 3:1. Respectively, that's 1 whole file write-read
...
ratio point \emph{between} our static configuration points (\ie{1:3, 1:1, and
3:1 described above}).
...
switching has between X and Y latency overhead compared to baseline I/O, with
average overhead at XX for 40M, 5M and 512K workloads and average overhead at YY
for 4K read workloads and YYY for 4K write workloads. There is no spatial
overhead with the Forward switching strategy.

Similarly, we calculate that Selective switching has between X and Y latency
overhead compared to baseline I/O, with average overhead at XX for 40M, 5M and
512K workloads and average overhead at YY for 4K read workloads and YYY for 4K
write workloads. Read overhead. Spatial overhead was half of all writable space
on the backing store.

Finally, we calculate that Mirrored switching has between X and Y read latency
overhead and between X and Y write overhead compared to baseline I/O, with
average overhead at XX/XXX for 40M, 5M and 512K read/write workloads and average
overhead at YY/YYY for 4K read/write workloads. Spatial overhead was half of all
writable space on the backing store.

\TODO{Crunch the numbers by hand!}
