...
the same dire implications for performance as simply re-initializing the entire
filesystem or encrypted container with the new cipher.
...
\TODO{Add text from notebook here.}

\subsection{Online Cryptographic Agility For High Throughput Systems}
...
plug-and-play way of putting things together. \TODO{Reword this entirely. Add
supporting paragraphs. Useful for switching primitives if one is found to be
insecure for whatever reason. This can be done online without taking the system
down and reinitializing the entire drive or array, which could be extremely time
consuming.}
...
with properties that meet current standards, \ie{future-proofing storage}.
