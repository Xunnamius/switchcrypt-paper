...
Even a single byte of additional length would make a cipher inappropriate for
...
1 if the ciphertext is always the same length as the plaintext. \hank{Would it
make sense to have this be a multiplier based on how much longer the ciphertext
is?  So if the cipher text is twice as long, the score is 2? I am a little
surprised that it is binary which would imply that all increase length by the
same amount. If that is true then we should say. }
