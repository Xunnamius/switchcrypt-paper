...
the other. When a nugget is encountered during I/O that is encrypted using a
cipher other than the active cipher, the forward strategy dictates that this
nugget be re-encrypted "just-in-time" using the active cipher.
...
encountered. Of course, this has the same effect as simply reinitializing the
filesystem with the new cipher.
