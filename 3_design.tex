...
\secref{motivation}). Ciphers without this property---such as ChaCha20 on which
...
is often (but not always) a necessary side-effect of ciphertext randomization. \hank{split the parentheticals to make it clear which examples apply to AES-XTS and which to ChaCha.}

This is a binary feature in that a cipher either outputs ciphertext of
...
the ciphertext is always the same length as the plaintext. \hank{Would
  it make sense to have this be a multiplier based on how much longer
  the ciphertext is?  So if the cipher text is twice as long, the
  score is 2? I am a little surprised that it is binary which would
  imply that all increase length by the same amount. If that is true then we should say. }
...
\hank{This all feels very abstract.  It would really help to make it
  concrete with a table or something that shows the different
  requirements for different ciphers.  Right now the interface is a
  major claim, but we don't have enough to justify that it is actually
  a hard problem.  (I know it is, but I mean from a reader/reviewer's
  perspective.)}
...
in a future read request. \hank{The lengths are specified as plaintext lengths?}
...
\hank{Putting my reviewer hat on here, it is not clear to me why we
  need three differenct levels. This list needs a sentence or two to
  preface what is going on.}
...
\begin{enumerate}
   ...
   with an unexpected cipher. \hank{This phrasing feels weird to me.
     Aren't we the implementers?  Are we the ones who have to account
     for this?  It sounds like a drawback, but it is not clear to me
...
\hank{How does this work with the rule that there is only one "active cipher?}
...
useful (see: \secref{usecases}). The near-instantaneous nature of SSD Instant
...
guarantees of the available ciphers used to encrypt nuggets. \hank{This last sentence needs to be rephrased because our motivational example describes a practical attack on existing ciphers that do not use output randomization. }
...
our enterprise backup service every so often. \hank{Probably need to say that we start with Freestyle to meet all the demands.  I have to be honest and say that even I forgot about that until I reread it here and I am sure that reviewers will be paying much less attention than me.}
...
\hank{I feel this part needs one last sentence to tie it all together.  Something about how using switchcrypt the system can now adapt to changing requirements. And maybe a forward reference to the associated case study.}
