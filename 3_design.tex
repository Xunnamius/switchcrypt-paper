...
\hank{This paragraph needs to be reworded (again for lazy reviewers). As written
  it would be too easy to get confused and think that Strongbox already solved
  the hard problems (really it is the first sentence that is the major cause of
  this issue).  Some subtle rephrasing can help draw a clear line between
  Strongbox and switchcrypt.  Something like ``Dickens et al. showed how to make
  this drive organization secure using a single stream cipher including (then
  list the things you are inheriting from that paper). However, they did not
  envision dynamically switching ciphers, so in the remainder of this section we
  show how to transform this prior approach by....''  Then I think you can
  insert something like that into this paragraph, keeping the bulk of the text,
  but subtly changing the way people percieve it.) } See Dickens et
  al.~\cite{StrongBox} for further discussion on handling overwrites, preventing
  rollback attacks, and limiting plaintext length using the nugget layout and
  aforesaid components. In the remainder of this section, we detail the primary
  components of the SwitchCrypt design: how we quantify the properties traded
  off between configurations (\cref{subsec:quantify}), the Generic Stream Cipher
...
\begin{table}[ht]
   \begin{tabular}{@{}lllll@{}}
   \toprule
   \textbf{Cipher} & \textbf{RR} & \textbf{CR} & \textbf{CE} \\ \midrule ChaCha8
   & 0           & 0           & 1           \\
   ChaCha12        & 0.5         & 0           & 1           \\
   ChaCha20        & 1           & 0           & 1           \\
   Salsa8          & 0           & 0           & 1           \\
...
\begin{enumerate}
   ...
   \item \textbf{\texttt{crypt\_data\_low}}\\\texttt{crypt\_data\_low} is
   identical to \texttt{crypt\_data} but provides a slightly lower level of
...
performance as simply re-initializing the entire system or encrypted container
with the new cipher.}
...
\begin{table}[ht]
   \begin{tabular}{@{}|c|c|c|C{25mm}|@{}}
      ...
      \textbf{Performance} \\ \midrule Forward   & Slow           & Low  & Fast
      reads and writes unless switching \\
      \hline
...
writes with low overhead except in the case where a nugget was not encrypted
with the active cipher configuration. This triggers costly re-ciphering, which
can compound depending on workload.\\
