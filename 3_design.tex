...
Dickens et al. showed how to make this drive organization secure using a single
stream cipher, ChaCha20, while handling overwrites, preventing rollback attacks,
and limiting plaintext length~\cite{StrongBox}. However, they did not envision
dynamically cipher switching or the utility in trading off between concerns at
the filesystem or device-mapper level; in the remainder of this section, we
detail the primary components that enable this functionality. Specifically: how
we quantify the properties traded off between configurations
(\cref{subsec:quantify}), the Generic Stream Cipher Interface and Per-Nugget
Metadata components (\cref{subsec:interface}) which decouple cipher
implementations from the encryption process, and our Cipher Switching Strategy
implementations (\cref{subsec:strategies}) used to efficiently encrypt nuggets
