...
\TODO{You need more of an overview here before the section starts in earnest.  It would be very good to have a picture where the components of the picture correspond to the subsections you write about here.  While the picture would help, whether you have it or not you need to say what are the key pieces of the design and how they inter-relate before you describe them in earnest.}

\subsection{Cipher Switching: Mechanism To Re-Cipher A Nugget}
...
\TODO{Again, we would like the language to be as general as possible so that this reads like it is more than just an extension of strongbox, but ideas that would be widely applicable.}
...
\TODO{This kind of high-level overview should not be in a subsection titled "Mechanism to Re-Cipher a Nugget."  If we are in such a subsection, the system design should already be known and we should just be focusing on nugget-level details.}
...
uses IPC to receive commands to toggle the active cipher configuration. \TODO{I believe this is an implementation detail and should be moved to an implementation section.  Also, it is a little weird to have it here.  We get an IPC for each nugget we want to recipher?  That seems really burdensome on the user.}

\TODO{I still don't know what the mechanism is.}
...
\TODO{It sure seems like this paragraph should have appeared earlier.}
...
\TODO{All this section says is that an API should exist.  It doesn't say what the API is.  That is a problem.}
...
\TODO{This paragraph is redundant and a good indication that the section needs a reorg and an overview paragraph or two.  If you find yourself needing to describe a mechanism (IPC in this case) in multiple places, you probably need a better overview paragraph.}
...
disadvantages of these strategies and each strategy.  \TODO{The table needs an explanation.  What are the columns, what are the meanings of the categories?  You probably need an overview of what the three strategies are as well.}
...
about secure erase here.} \TODO{Hank: I think we should use that example, and I think we should talk about the secure erase.  I assume that is how you get Nearly Instant in the convergence category.}
...
executing, \ie{long sequential writes vs write-once read-many}.
...
\TODO{It is a little concerning that this "additional challenge" has a longer section than some of the "main" contributions.}
...
\TODO{You should mention (unless you don't agree) that the scores we use are a combination of two schemes that are already in use: ranking ciphers on resistance to different attacks and ranking ciphers on "rounds" or whatever.  So, we should say that all we are doing here is combining them.}
...
SwitchBox FDE (see: \tblref{security-quant}).\\
\\
