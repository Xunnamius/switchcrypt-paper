...
\PUNT{\begin{figure}[ht]
   ...
   \caption{Anatomy of a SwitchCrypt nugget.}\label{fig:flknug}
\end{figure}}

As with prior work, SwitchCrypt divides the underlying drive into a series
...
Layer (FTL). \TODO{Why is that ideal? And where else could it be implemented? Remember that you want all the ideas to have as much impact as possible.  Don't give lazy reviewers and easy lever against the paper.} 
...
\figref{overview} provides an overview of the SwitchCrypt design. SwitchCrypt
...
\begin{enumerate}
   ...
   below the overwrite protection code (\ie{Freestyle}) and/or a cipher does not
   operate like a stream cipher (\ie{AES-XTS}). Hooks at this level should only
...
SwitchCrypt allows each individual nugget to exist encrypted with any cipher the
generic cipher API makes available regardless of the currently active cipher
...
intuitive is \emph{0-forward}, in which SwitchCrypt immediately transitions
...
The Forward strategy might also take the form of \emph{N-forward}, where SwitchCrypt
...
equal to the total number of nuggets managed by SwitchCrypt. This would have the
...
When SwitchCrypt is initialized with the Selective strategy, the backing store is
...
and 1 is enhanced resistance. \TODO{Enhanced how?}\\
\\
