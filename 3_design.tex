...
\subsection{\sys Overview} \label{subsec:overview}

\begin{figure}[ht]
   \centering
   \includegraphics[width=\linewidth]{overview.png}
   \caption{Overview of the \sys construction.} \label{fig:overview}
\end{figure}

\sys consists of a \emph{Generic Stream Cipher Interface} and
\emph{Cryptographic Driver}; \sys sits between a Log-structured File
...
\figref{overview}, which provides an overview of the \sys system design.

\begin{figure}[t]
   \centering
   \includegraphics[width=\linewidth]{backstore.png}
   \caption{Layout of \sys's drive layout.} \label{fig:backstore}
\end{figure}

\hank{The part that is missing here is some kind of motivation for
  this level of detail.  For most reviewers/readers going into this
  level of technical detail will be jarring without some kind of
  high-level explanation of why they should care.  There must be some
  reason that it is important to understand how the drive is laid out
  and that the body section is broken down into chumks.  As currently
  constructed, you have to wade into that detail before you get the
  reason, and even then it is presented as a reason for StrongBox to
  do this, not necessarily as a reason that \sys should keep
  it.
  \\
  \\
  So, I want something of the form: The key insight behind \sys
  is achieving low-overhead cipher-switching by exploiting fine-grain
  tracking and manipulation of the cipher used to encrypt different
  drive regions.  While existing stream cipher FDE schemes implement
  much of the required tracking, their design does not accomodate
  dynamic manipulation.  Therefore, we first review the structure of
  such an existing approach and then describe how we extend it to add
  the required functionality for SwitchCrupt.
  \\
  \\
  I am guessing that you can find a way to phrase that more eloquently
  than I did here, but hopefully you get the high-level idea: tell
  your readers where you are going so that they will happily join you
  for the ride.  Then, obviously, if you start it this way, you will
  need to rephrase parts of the rest of this section given that you
  have already introduced this high-level goal. Overall, I think that
  the total text will stay about the same, I just want you to hoist
  parts of it up to the beginning.}
...
\hank{There is one major problem with this section: The scores are
  presented without any methodology for coming up with them (with the
  exception of length preservation). As with any empiricial results
  that we would present, our standard should be that any numbers we
  present are reproducible.  That is to say, any reader should come up
  with the same scores that we do if they did not see them ahead of
  time.  For example, there should be a clear reason why Freestyle(B)
  is a 2.5 on randomization instead of 2.4 or 2.6.  Also there is no
  cipher listed as 1 for that feature; why?  Why does ChaCha12 get .5
  for Rounds? Is it really halfway between 8 and 20?  Whatever the
  reason, that needs to be explained. For expansion, why is it binary
  instead of something related to how much it expands?  Do you view a
  theoretical cipher that doubles the length as equivalent to one that
  increases it by 1.5x?
  \\
  \\
  Also, looking ahead to section 4, you are going to use the scores,
  and call it the R+R vector. If you like that name, you should
  introduce it here and make an argument about how you can combine
  Rounds and Randomization into one coherent metric.  However, this
  also raises the question of why this section appears to penalize
  ciphers for expansion, yet that is not captured in this composite
  score.  Basically, this is another question of methodology where the
  scoring and comparsions don;t have enough information to be
  reporoducible. }


To reason about when to trade off between the ciphers evaluated in
this work, we must have a way to compare ciphers' utility in the
context of \sys FDE.  However, different ciphers have a wide
range of security properties, performance profiles, and output
characteristics. \hank{To this point in the paper we haven't really
  listed any of the practical concerns surrounding these security
  properites.  That makes it a little hard to understand why we need
  this complicated scoring scheme.} To address this need, we propose a
novel evaluation framework (see: \tblref{security-quant}). Our
framework classifies stream ciphers according to three quantitative
features: relative round count, ciphertext randomization, and
ciphertext expansion. Taken together, these features reveal a rich
tradeoff space of cipher configurations optimizing for different
...
\caption{The five most interesting ciphers examined in this research, each
  ...
  \hank{Why did you take the others out?  I think it is interesting to keep them in.  
    If you are worried about space, I am sure that we can find 6 more lines of space.  
    It certainly makes this part seem like more of a contribution because it applies generally.}}
...
We limit our analysis to groups of three implementations, each using a
...
considered). \hank{This text is inconsistent with the table.  Either
  include all the ciphers in the table (my preference) or don't
  include ciphers in the text that aren't in the table.}
...
block). \hank{I really puzzled over this and I cannot figure out what
  a 2.5 means here or how I would get that.}
...
\hank{Speaking with my reviewer hat on: There is good motivation for
  the difficulty of working with different stream ciphers, but it is
  not clear why this interface is a good solution to that challenge.
  I get the impression that the xor interface is easier to use, while
  the other one just passes all the complexity to someone else. Also,
  for naming (assuming I correctly understand the difference between
  these) it is confusing to have xor (a very low-level operation)
  correspond to the high-level interface and read/write (high-level
  operations) correspond to the low-level interface.}
...
Unlike prior work, \sys must be able to encrypt and decrypt
...
process. \hank{This sounds good, but I am not sure exactly what it
  means.  We need to define what decoupling means so that a reader
  would be able to point to an interface and correctly say whether it
  is decoupled or not.} Our novel cipher interface allows any stream
...
in our tradeoff space efficiently and effectively.  \hank{One thing
  that worries me a little (with reviewr hat on) is that we use two
  ciphers in this paper and we have two interfaces, one which appears
  to be appropriate for ChaCha and one for FS.  The appearance is that
  we did not actually decouple the interface from the cipher but just
  created two interfaces for the two ciphers.}
...
\begin{enumerate}
 ...
 \item \textbf{\texttt{read\_interface}} and
   ...
   complexity, as the implementer \hank{WHo is the implementer here?}
...
\hank{I have to admit I am confused here.  The beginning of this
  section goes into detail about how the layout of the body section is
  important and then there is an interface that completely bypasses
  it.  So is the layout important or not? The performance implications
  of the different approaches are entirely unclear to me, while I can
  take your word for it, I don't really understand when one would be
  faster than another.}
...
most intuitive is \emph{0-forward}, in which \sys immediately transitions
...
\sys attempts to take advantage of spatial sequential locality to
...
to the total number of nuggets managed by \sys. This would have the
...
We revisit the motivating example from earlier in this work, where we
...
enterprise backup service every so often. \hank{I really want a
  clarification in the previous sentence.  There are two interfaces in
  the Generic Stream Cipher Interface, so which one is called and why?}
...
Now, thanks to \sys, the system can adapt to changing requirements beyond
...
\hank{AGain, I find it a bit confusing that we propose a generic
  interface but them have to customize it for the ciphers we use in
  this paper.  Something feels off here, and I am not sure how to fix
  it.  It could be that the name of this interface needs to change, or
  maybe just the description of what it does and who uses it. I want
  to be very clear that there is no problem with the actual work that
  has been done.  The problem is that the description of the work
  leaves unanswered questions about exactly what problems this has
  solved.}
