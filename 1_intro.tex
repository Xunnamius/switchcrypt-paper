...
%Prior work requires a static
%configuration choice made offline at compile- or boot-time---a single
%cipher configuration is chosen to work generally well for all possible scenarios that might arise at runtime.
...
\TODO{Hank: I still think we could improve the flow in this paragraph, but we can fine tune later.}

While configuration parameters allow filesystems to balance security,
...
usecases? Or save that for motivation?} \TODO{Hank: MY suggestion is that this intro is very compact and to the point right now.  I think the current structure would not need this paragraph, but woudl be well suited to a more fleshed out motivation section where you show that flexiblity really is useful in some areas.  So, I am suggesting omitting the paragraph stub and going right to the brief description of switchbox.}
