...
However, the standard cipher choice for FDE is the slow AES
...
(AES-CTR), Rabbit, Sosemanuk, etc~\cite{Freestyle, SalsaX, Rabbit,
  Sosemanuk, ChaCha20, AESCTR}. \hank{REorder this so that the
  citations for each cipher appear right after the name of that cipher
  in the sentence.} Given this variety, the cipher configuration
...
Hence, any static common-case FDE configuration will sacrifice one
...
configurations at runtime. \hank{I would like to see these three
  contributions in a bulleted list so that even lazy reviewers will
  not be confused about how many contributions we claim, nor precisely
  what they are.}
